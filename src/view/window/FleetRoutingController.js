...
Ext.define('Ors.window.FleetRoutingController', {
  ...
  requires: [
    'Ext.Array',
    'Ors.util.VroomFleetRouting',
    'Ors.util.OpenRouteService',
    'Ors.util.Geocoding',
    'Ors.util.RoutingVehicles',
    'Ors.util.Export',
    'BasiGX.util.Layer'
  ],
  ...
  */
  ...
  optimizeRoute: function () {
    ...
    var vehiclesGrid = view.down('ors-grid-routing-vehicles');
    if (!vehiclesGrid) {
      return;
    }
    var vehicleStore = vm.get('routingvehicles');
    var vehicleProfileBtn = vehiclesGrid.down('ors-button-routing-profile');
    ...
    Ors.util.VroomFleetRouting.performOptimization(vehicles, jobs, avoidArea)
  ...
  setJobAdress: function (address) {
    var jobWindow = Ext.ComponentQuery.query('ors-window-routing-job')[0];
    if (jobWindow) {
  ...
  setVehicleStart: function (address) {
    var vehicleWindow = Ext.ComponentQuery.query('ors-window-routing-vehicle')[0];
    if (vehicleWindow) {
  ...
  addJobWithAddress: function (address) {
    ...
    var jobsGrid = view.down('ors-grid-routing-jobs');
    if (jobsGrid) {
  ...
  triggerReverseGeocoding: function (longitude, latitude) {
    ...
    var geocoding = Ors.util.Geocoding;
  ...
  handleJobUpload: function (evt) {
    ...
    view.setLoading(true);
    var GeocodingUtil = Ors.util.Geocoding;
  ...
  handleVehicleUpload: function (evt) {
    ...
    var RoutingVehiclesUtil = Ors.util.RoutingVehicles;
  ...
  exportJobs: function () {
    ...
    var exportFile = Ors.util.Export.exportFile;
  ...
  onAvoidAreaDrawEnd: function () {
    ...
    var routingWindow = view.up('ors-window-fleet-routing');
    if (routingWindow) {
